...
Also, last week we talked about the Serial Peripheral Interface protocol, or SPI, and mentioned how the function we wrote to flash wires on and off is basically the same as that protocol. For this version, I had you wire the clock and data lines to the SPI's SCK and MOSI lines, which means that you could get rid of those pin definitions, and run the clock using the SAMD11's native SPI support. This (counterintuitively) increases the code size a bit, since it has to account for other modes and options for SPI, but the code still fits, and it's worth experimenting with. [Here's an Arduino sketch with that code](/assets/code/2020-03-05-Flash_SAMD11_Bootloader.zip). 
